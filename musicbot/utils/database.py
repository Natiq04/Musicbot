...
autorestartdb = mongodb.autorestart
...
class AutoRestart:
    def __init__(self):
        self.db: Collection = autorestartdb

    async def get(self) -> dict:
        data = await self.db.find_one({"_id": "restart"})
        if not data:
            return {"status": False, "interval": 0}
        data.pop("_id")
        return data

    async def set(self, restart: bool = None, interval: int = None):
        data: dict = await self.get()
        if restart is not None:
            data["status"] = restart
            autorestart["status"] = restart
        if interval is not None:
            data["interval"] = interval
            autorestart["interval"] = interval

        await self.db.update_one({"_id": "restart"}, {"$set": data}, upsert=True)

    async def get_status(self) -> bool:
        if "status" in autorestart:
            return autorestart["status"]
        data = await self.get()
        autorestart["status"] = data["status"]
        return data["status"]

    async def get_interval(self) -> int:
        if "interval" in autorestart:
            return autorestart["interval"]
        data = await self.get()
        autorestart["interval"] = data["interval"]
        return data["interval"]

    async def set_status(self, restart: bool):
        await self.set(restart=restart)

    async def set_interval(self, interval: int):
        await self.set(interval=interval)
